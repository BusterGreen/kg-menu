...
RegisterNetEvent('KGCore:Client:UpdateObject', function() KGCore = exports['kg-core']:GetCoreObject() end)
...
local function openMenu(data, sort, skipFirst)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			if KGCore.Shared.Items[tostring(v["icon"])] then
				if not string.find(KGCore.Shared.Items[tostring(v["icon"])].image, "//") and not string.find(v["icon"], "//") then
                    v["icon"] = "nui://kg-inventory/html/images/"..KGCore.Shared.Items[tostring(v["icon"])].image
				end
			end
		end
	end
...
RegisterNetEvent('kg-menu:client:openMenu', function(data, sort, skipFirst)
    openMenu(data, sort, skipFirst)
end)

RegisterNetEvent('kg-menu:client:closeMenu', function()
    closeMenu()
end)
    ...
    if sendData then
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isKGCommand then
                    TriggerServerEvent('KGCore:CallCommand', data.params.event, data.params.args)
...
RegisterNUICallback('closeMenu', function(_, cb)
    ...
    SetNuiFocus(false)
    cb('ok')
    TriggerEvent("kg-menu:client:menuClosed")
end)
